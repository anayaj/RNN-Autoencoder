...
# Specify filenames
# Root directory:
dir_name = "C:/Users/anaya/Dropbox/UPENN/SeniorFall/COGS398/Jonzia_AE/"
with tf.name_scope("Training_Data"):	# Training dataset
	tDataset = os.path.join(dir_name, "data/data.csv")
with tf.name_scope("Validation_Data"):	# Validation dataset
	vDataset = os.path.join(dir_name, "data/data.csv")
...
decoder_cell_1 = tf.nn.rnn_cell.LSTMCell(lstm_decoder.num_lstm_hidden,name='Decoder_Cell_1')
decoder_cell_2 = tf.nn.rnn_cell.LSTMCell(lstm_decoder.num_lstm_hidden,name='Decoder_Cell_2')
...
with tf.Session() as sess:
	...
	for step in range(0,step_range):
	    ...
	    ###change to True if you want to do PCA/tSNE analysis
		if False:
			plotPCAtSNE()
